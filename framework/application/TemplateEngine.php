...
abstract class TemplateEngine extends Base
{
    ...
    final public function GetTemplateParameters($option, $tag_name) 
    {
        ...
        /** Function name is converted to camelcase */
        ...
        if (is_callable($template_parameters_callback)) $template_parameters = call_user_func($template_parameters_callback);
        else $template_parameters = array();
     ...
     *    1 => list of tags values that will replace the tags in the given template file
     */
    final public function GetTemplateTagsFromFunction($tag_name) 
    {
        ...
        if (!isset($application_url_mappings[$application_option])) throw new \Exception("Invalid url request sent to application");
        /** If the function type is template then application url template mapping is auto generated */
        if ($application_url_mappings[$application_option]['function_type'] == 'template') 
        {
            /** The callback object and function are fetched */
            $callback = $this->GetComponent("application")->GetOptionCallback($application_option);
            /** The application url template mapping */
            $url_templates = array(
                array(
                    "tag_name" => $tag_name,
                    "template_file_name" => $application_url_mappings[$application_option]['template_file_name'],
                    "object_name" => $callback['object_name'],
                    "function_name" => $callback['function'],
                    "use_presentation" => $application_url_mappings[$application_option]['use_presentation'],
                )
            );
        }
        /** If the function type is recursive template then the url template mapping is fetched from configuration */
        else 
        {
        ...
        for ($count = 0;$count < count($url_templates);$count++) 
        {
            ...
            if ($url_templates[$count]["tag_name"] == $tag_name) 
            {
                ...
                /** Used to indicate if the callback function should use template parameters returned by a presentation class function */
                $use_presentation = $url_templates[$count]['use_presentation'];
            }
        }
        /** If the tag name was not found in the url template mapping then the callback function is generated and called */
        if (!isset($template_file_name))
        {
            ...
            /** Used to indicate if the callback function should use template parameters returned by a presentation class function. If the tag name was not found in the url mapping, then the use_presentation is set to the default use_presentation value */
            $use_presentation = $this->GetConfig("general", "default_use_presentation");
        }
        /** The callback object */
        $ui_object = $this->GetComponent($ui_object_name);
        /** The parameters that are passed to the callback function */
        $template_parameters = array();
        /** If the template callback function requires presentation feature then the presentation class function is called */
        if ($use_presentation) 
        {
        ...
        if (is_callable($template_callback)) $tag_replacement_arr = call_user_func_array($template_callback, array(
            $template_file_name,
            $template_parameters
        ));
    ...
    final public function RenderApplicationTemplate($tag_name) 
    {
        ...
        for ($count = 0;$count < count($template_tag_list);$count++) 
        {
            ...
            if (is_array($tag_value)) $tag_value = $this->GetComponent("template_helper")->ReplaceTagWithArray($tag_name, $tag_value);
